...
} from "../scaffolding/configurationScaffolder";
import {
	ConflictHandler,
	ConflictUniquenessPredicate,
} from "../scaffolding/conflicts";
			...
			if (launchSettingsJson.profiles) {
				...
				if (projectProfile?.applicationUrl) {
					const applicationUrls =
						projectProfile.applicationUrl.split(";");
...
export async function scaffoldDaprTasks(
	...
	if (
		...
		const configurationStep: WizardStep<ScaffoldWizardContext> = async (
			...
			const configurationItems = workspaceConfigurations.map(
				(configuration) => ({
			...
			const debugConfigurationItem = await ui.showQuickPick(
				configurationItems,
				{
		...
		const result = await ui.showWizard(
			{
				title: localize(
...
const createScaffoldDaprTasksCommand =
	(
		scaffolder: Scaffolder,
		templateScaffolder: TemplateScaffolder,
		ui: UserInput,
