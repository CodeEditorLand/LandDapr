...
async function getAppIdsToDebug(
	configuration: DaprDebugConfiguration,
): Promise<Set<string>> {
	...
	const runFile = await fromRunFilePath(
		vscode.Uri.file(configuration.runFile),
	);
	...
	for (const app of runFile.apps ?? []) {
		...
		if (
			configuration.excludeApps === undefined ||
			!configuration.excludeApps.includes(appId)
		) {
			appIds.add(appId);
		}
	}
...
export class DaprDebugConfigurationProvider
	implements vscode.DebugConfigurationProvider
{
	...
	async resolveDebugConfigurationWithSubstitutedVariables?(
		...
		debugToken?: vscode.CancellationToken,
		...
		await this.userInput.withProgress(
			localize(
				"debug.daprDebugConfigurationProvider.attachingProgress",
				"Attaching to Dapr applications...",
			),
			(process, userToken) =>
				withAggregateTokens(
					(token) =>
						this.attachToApplications(
							daprDebugConfiguration.runFile,
							appIds,
							token,
						),
					userToken,
	...
	private async attachToApplications(
		runFile: string,
		appIds: Set<string>,
		token: vscode.CancellationToken,
	): Promise<void> {
		const applications = await this.waitForApplications(
			runFile,
			appIds,
			token,
		);
	...
	private async waitForApplications(
		runFile: string,
		appIds: Set<string>,
		token: vscode.CancellationToken,
	): Promise<DaprApplication[]> {
		const applicationMonitor =
			this.daprApplicationProvider.applications.pipe(
				// Include only those applications that are part of the run...
				map((apps) =>
					apps.filter((app) => app.runTemplatePath === runFile),
				),
		...
		const applications = await firstValueFrom(
			race(applicationMonitor, fromCancellationToken(token)),
		);
		...
		if (!applications) {
			throw new Error(
				localize(
					"debug.daprDebugConfigurationProvider.noApplicationsRunning",
					"No applications were found to be running.",
				),
			);
		}
	...
	private async waitForAttach(
		...
		if (hadFailures) {
			throw new Error(
				localize(
					"debug.daprDebugConfigurationProvider.hadFailures",
					"The debugger failed to attach to all applications.",
				),
			);
		}
	}
