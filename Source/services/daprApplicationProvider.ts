...
export default class DaprListBasedDaprApplicationProvider
	implements DaprApplicationProvider
{
	...
	async getApplications(): Promise<DaprApplication[]> {
		...
		let applications = JSON.parse(applicationsJson) as
			| DaprListApplication[]
			| DaprListApplication;
		...
		return applications
			.map((application) => ({
				...
				appPid:
					application.appPid !== 0 ? application.appPid : undefined,
