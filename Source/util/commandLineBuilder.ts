...
export default class CommandLineBuilder {
	...
	public withArg(
		...
		if (typeof arg === "string") {
			if (arg) {
				// Quoted strings can be added as empty, but withArg will not allow an empty string arg
				this.args.push({
					value: arg,
	...
	public build(): string {
		return this.args
			.map((arg) => {
				return arg.quoting === vscode.ShellQuoting.Strong
					? `"${arg.value}"`
					: arg.value;
			})
			.join(" ");
